import { PrismaClient, TagName, EvidenceStrength } from '@prisma/client';
...
async function main() {
  ...
  // Create tags
  const bdsTag = await prisma.tag.upsert({
    where: { id: 'bds-tag' },
    update: {},
    create: {
      id: 'bds-tag',
  ...
  const defenseTag = await prisma.tag.upsert({
  ...
  const surveillanceTag = await prisma.tag.upsert({
    where: { id: 'surveillance-tag' },
    update: {},
    create: {
      id: 'surveillance-tag',
      name: TagName.SURVEILLANCE,
  ...
  const vti = await prisma.company.upsert({
    where: { ticker: 'VTI' },
    update: {},
    create: {
      ticker: 'VTI',
  ...
  await prisma.financial.upsert({
    where: { id: 'coca-financials-2024' },
    update: {},
    create: {
      id: 'coca-financials-2024',
      companyId: cocaCola.id,
      period: '2024-Q4',
      debt: 40000000000, // $40B
      cashSecurities: 10000000000, // $10B
      receivables: 5000000000, // $5B
  ...
  await prisma.eTFHolding.upsert({
    where: { id: 'vti-lockheed' },
    update: {},
    create: {
      id: 'vti-lockheed',
      etfId: vti.id,
      childIsinOrTicker: 'LMT',
      weight: 0.3,
      lookthroughLevel: 1,
      asOf: new Date(),
    },
  ...
  // Create screen rules
  await prisma.screenRule.upsert({
    where: { id: 'bds-rule-001' },
    update: {},
    create: {
      id: 'bds-rule-001',
      filter: 'BDS',
      code: 'AFSC_OCCUPATION',
      description: 'AFSC occupation links check',
      thresholdJson: { evidenceStrength: ['MEDIUM', 'HIGH'] },
      version: '1.0',
      active: true,
    },
  });
  ...
  await prisma.screenRule.upsert({
    ...
    create: {
      ...
      thresholdJson: { sipriRankThreshold: 100 },
      version: '1.0',
      active: true,
    },
  });
